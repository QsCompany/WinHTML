...
( function ( ba ) {
    // @b isFunction :boolean
    function isFunction(b){ return b instanceof Function; }
    // @b isArray :boolean
    ...
    // if @c is ownProperty of @b $return $b.c $else $return false //todo: b.c is always an object assembly
    function getOwnProperty(b, c){ return b.hasOwnProperty(c) ? b.c : null; }

    // EXCLUSIVE  mybe is parcure
    function B(b, c){
    ...
    function U(b, c, d, e){
        c&&B(c, function(c, g) {
            if(d||!t(b, g))
                e&&"object"===typeof c&&c&&!isArray(c)&&!isFunction(c)&&!(c instanceof
    ...
    function ga(b){
                ...
                a:for(;0<c;c-=1){
                    ...
                    if(k)
                        for(d=k.length;0<d;d-=1)
                            if(b=getOwnProperty(h, k.slice(0, d).join("/")))
                                if(b=getOwnProperty(b, e)){
                                    f = b;
                                    g = c;
                                    break a
                                }
                    !i&&(n&&getOwnProperty(n, e))&&(i=getOwnProperty(n, e), p=c)
                }
                !f&&i&&(f=i, g=p);
                f&&(l.splice(0,
                    g, f), a=l.join("/"))
            }
            return(f=getOwnProperty(j.pkgs, a)) ? f : a
        }

        function d(a){ z&&T(document.getElementsByTagName("script"), function(k) { if(k.getAttribute("data-requiremodule")===a&&k.getAttribute("data-requirecontext")===i.contextName) return k.parentNode.removeChild(k), !0 }) }

        function e(a){
            var k = getOwnProperty(j.paths, a);
            if(k&&isArray(k)&&1<k.length) return k.shift(), i.require.undef(a), i.makeRequire(null, { skipMap :!0 })([a]), !0
        }
        ...
        function p(a,k, b, f){
            ...
            e = a[0];
            a = a[1];
            e&&(e=c(e, g, f), d=getOwnProperty(r, e));
        ...
        function w(a, b){
            var c = a.requireModules,
                f = !1;
            if(b) b(a);
            else if(T(c, function(b) { if(b=getOwnProperty(h, b)) b.error = a, b.events.error&&(f=!0, b.emit("error", a)) }), !f) g.onError(a)
        }
        ...
        function F(a, b, c){
            var f = a.map.id;
            a.error ? a.emit("error", a.error) : (b[f]=!0, T(a.depMaps, function(f,
                                                                                 d) {
                var e = f.id,
                    g = getOwnProperty(h, e);
                g&&(!a.depMatched[d]&&!c[e])&&(getOwnProperty(b, e) ? (a.defineDep(d, r[e]), a.check()) : F(g, b, c))
            }), c[f]=!0)
        }
        ...
        L = {
            ...
            module :function(a){
                return a.module ?
                           a.module : a.module={ id :a.map.id, uri :a.map.url, config :function(){ return getOwnProperty(j.config, a.map.id)||{} }, exports :a.exports||(a.exports={}) }
            }
        };
        Z = function(a){
            this.events = getOwnProperty($, a.id)||{};
            this.map = a;
            this.shim = getOwnProperty(j.shim, a.id);
            this.depExports = [];
            this.depMaps = [];
            this.depMatched = [];
            this.pluginMaps = {};
        ...
        Z.prototype = {
            ...
            callPlugin :function(){
                ...
                q(d, "defined", u(this, function(f) {
                    var l,
                        d;
                    d = getOwnProperty(aa, this.map.id);
                    var e = this.map.name,
                        P = this.map.parentMap ? this.map.parentMap.name : null,
                        n = i.makeRequire(a.parentMap, { enableBuildCallback :!0 });
                    if(this.map.unnormalized){
                        if(f.normalize&&(e=f.normalize(e, function(a) { return c(a, P, !0) })||""), f=p(a.prefix+"!"+e, this.map.parentMap), q(f, "defined", u(this, function(a) { this.init([], function() { return a }, null, { enabled :!0, ignore :!0 }) })), d=getOwnProperty(h, f.id)){
                            this.depMaps.push(f);
                            if(this.events.error) d.on("error", u(this, function(a) { this.emit("error", a) }));
                            d.enable()
                        }
                    } else
                        d ? (this.map.url=i.nameToUrl(d), this.load()) : (l=u(this, function(a) { this.init([], function() { return a }, null, { enabled :!0 }) }), l.error=u(this, function(a) {
                            this.inited = !0;
                            this.error = a;
                            a.requireModules = [b];
        ...
        i = {
            ...
            configure :function(a){
                ...
                a.bundles&&B(a.bundles, function(a, b) { T(a, function(a) { a!==b&&(aa[a]=b) }) });
                a.shim&&(B(a.shim, function(a, c) {
                    isArray(a)&&(a={ deps :a });
                    if((a.exports||a.init)&&!a.exportsFn) a.exportsFn = i.makeShimExports(a);
                    b[c] = a
                }), j.shim=b);
                a.packages&&T(a.packages, function(a) {
                    var b,
            ...
            makeRequire :function(a, e){
                function j(c, d, m){
                    var n,
                        q;
                    e.enableBuildCallback && (d && isFunction(d)) && (d.__requireJsBuild = !0);
                    if("string" === typeof c){
                        if(isFunction(d)) return w(C("requireargs", "Invalid require call"), m);
            ...
            enable :function(a){ getOwnProperty(h, a.id)&&s(a).enable() },
            completeLoad :function(a){
                var b,
                    c,
                    d = getOwnProperty(j.shim, a)||{},
                    g = d.exports;
                for(x();A.length;){
                    c = A.shift();
                    if(null===c[0]){
            ...
            nameToUrl :function(a, b, c){
                var d,
                    e,
                    h;
                (d=getOwnProperty(j.pkgs, a))&&(a=d);
                if(d=getOwnProperty(aa, a)) return i.nameToUrl(d, b, c);
    ...
    if("undefined" === typeof define){
        ...
        "undefined" !== typeof require && !isFunction(require) && (q = require, require = void 0);
        g = requirejs = function(b, c, d, e){
            var n,
                p = "_";
            !isArray(b) && "string" !== typeof b && (n = b, isArray(c) ? (b = c, c = d, d = e) : b = []);
            n && n.context && (p = n.context);
            (e = getOwnProperty(F, p)) || (e = F[p] = g.s.newContext(p));
            n && e.configure(n);
        ...
        define = function(b, c, d){
            ...
            if(!c && isFunction(d)){
                c = [];
                if(d.length != 0){
                    d.toString().replace(ka, "").replace(la, function(b, d) { c.push(d) });
                    if(1 == d.length) c = ["require", c];
                    else c = ["require", "exports", "module", c];
                }
            }
            if(M){
                if(e != J){
                ...
                if(e){
                    if(!b) b = e.getAttribute("data-requiremodule");
