import Base = require("../app");
import m = require('./Slideout');
...
export module my.controls {
    ...
    export class Slideout extends Emitter {
        private _startOffsetX = 0;
        private _currentOffsetX = 0;
        private _opening = false;
        private _moved = false;
        private _opened = false;
        private _preventOpen = false;
        private _closing = false;
        private _touch;
        private _thread: number = -1;
        ...
        private _translateTo: number;

        constructor(options: any) {
            super();
            if (options.panel == null) return;
         ...
         */
            this._onTouchMoveFn = function (eve) {
                ...
                if (Math.abs(dif_x) > 20) {
                    ...
                    (<any>self.emit)('translate', translateX);
                    self._moved = true;
                }
     ...
     */
        ...
        private _onScrollFn;
        private _preventMove: (eve) => void;
        private _onTouchMoveFn: (eve) => void;
        private _onTouchEndFn: () => void;
        private _onTouchCancelFn: () => void;
        private _resetTouchFn: (eve) => void;
    }

    export class PSlideout extends Slideout {
        private toggles: HTMLElement[] = [];

        constructor(menu, panel, toggle: HTMLElement) {
            super({
                'panel': panel,
                'menu': menu,
                'duration': 1000
            ...
            this
                .on('beforeopen', () => { this.menu.style.display = 'block'; })
                .on('close', function () {
                    this.menu.style.display = 'none';
                    menu.removeEventListener('mouseleave', close);
                })
                .on('open', () => { menu.addEventListener('mouseleave', close); });
            menu.addEventListener('mouseenter', function () { if (self.isClosing()) self.open(); });
        ...
        public toggle() {
            ...
            else PSlideout.OpenByCloseOthers(this, (<any>this.panel).menus, 0);
            return this;
        }

        static OpenByCloseOthers(self: PSlideout, slides: PSlideout[], i: number) {
            if (i >= slides.length) {
                self.open();
                return;
            }
